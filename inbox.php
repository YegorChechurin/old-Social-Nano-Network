<?php
    ...
    // require_once('BT_chats_view.php');
    // if (!empty($_POST)) {
	// 	$user->send_mes($_POST);
	// 	var_dump($_POST);
	// }
...
<head>
	...
	<script>
	    ...
	    <?php foreach($chats as $chat): ?> 
		    $("#chat<?=$chat['key']?>").click(
			    function(){
			    	active_chat_key = <?=$chat['key']?>;
			    	rec_id = <?=$chat['destination']?>;
					document.getElementById("mes").innerHTML = "";
					<?php foreach($chat['messages'] as $key => $value): ?> 
					    <?php if($value['direction']=='received'): ?>
				            document.getElementById("mes").innerHTML += '<div class="message_inlet"><b>Your friend:</b> <?=$value['message']?></div><br>';
						<?php else: ?>
						    document.getElementById("mes").innerHTML += '<div class="message_outlet"><b>You:</b> <?=$value['message']?></div><br>';
						<?php endif; ?>
					<?php endforeach; ?>
                }
		    );
    	...
    	/*function send(rec_id) {
            ...
            xhttp.send();
            document.getElementById("mes").innerHTML += '<div class="message_outlet"><b>You:</b> '+message+'</div><br>';
                ...
                if (this.readyState == 4 && this.status == 200) {
                	if (this.responseText) {
                		...
                		if (mes.mes_key == active_chat_key) {
                			document.getElementById("mes").innerHTML += '<div class="message_inlet"><b>Your friend:</b> '+mes.message+'</div><br>';
                		}
                	}
                }
            };
            ...
            ajax.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                	if (this.responseText) {
                		...
                		var n = mes.length;
                		last_rec = mes[n-1].message_id;
                		if (mes.mes_key == active_chat_key) {
                			document.getElementById("mes").innerHTML += this.responseText;
                			//'<div class="message_inlet"><b>Your friend:</b> '+mes.message+'</div><br>';
                		}
                	}
                }
            };
	  ...
	  <div class="row">
		<div class="col-sm-6 col-sm-offset-3">
			<form method="post" id="message" action="inbox.php">
                ...
                <input type="button" class="btn btn-info" onclick="fresh()" value="Reload">
